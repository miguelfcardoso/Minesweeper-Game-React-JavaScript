...
export default function Board({
    ...
    const handleCellClick = (x, y) => {
        ...
        if(grid[x][y].hasQuestion){
            grid[x][y].hasQuestion=false;
        }
    ...
    const openAdjacentCells = (x, y, updatedGrid) => {
        ...
        for (const direction of directions) {
            ...
            if (isValidCell(newX, newY, updatedGrid.length, updatedGrid[0].length) && !updatedGrid[newX][newY].isOpen && !updatedGrid[newX][newY].isBomb) {
                ...
                if(updatedGrid[newX][newY].hasFlag){
                    updatedGrid[newX][newY].hasFlag = false;
                    flagCount++;
                }
                
                if(updatedGrid[newX][newY].hasQuestion){
                    updatedGrid[newX][newY].hasQuestion = false;
                }
